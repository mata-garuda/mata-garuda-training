...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    /**
     * Define allow for exclude method
     */
    ...
    /**
     * Define variable for support rest controller 
     *
     */
    ...
    public function set_config($action, $c) {
        $c = round(($c/$this->limit));
    ...
    public function __construct($config = 'rest')
    {
        ...
        if (config_item('rest_database_group') && (config_item('rest_enable_keys') || config_item('rest_enable_logging'))) {
            $this->restserver->db     = $this->load->database(config_item('rest_database_group'), TRUE);
        }

        // Use whatever database is in use (isset returns FALSE)
        elseif (property_exists($this, "db")) {
            $this->restserver->db     = $this->db;
        }

        // Check if there is a specific auth type for the current class/method
        // _auth_override_check could exit so we need $this->restserver->db initialized before
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (config_item('rest_enable_keys') && $use_key && $this->_allow === FALSE) {
            ...
            $this->response([config_item('rest_status_field_name') => FALSE, config_item('rest_message_field_name') => 'Invalid API Key '.$this->restserver->key], 403);
        }

        // Check to see if this key has access to the requested controller.
        if (config_item('rest_enable_keys') && $use_key && !empty($this->restserver->key) && !$this->_check_access()) {
            if (config_item('rest_enable_logging') && $log_method) {
                $this->_log_request();
            }
        ...
        if (config_item('rest_enable_keys') && !empty($this->restserver->key)) {
            // Check the limit
            ...
            $authorized = $level <= $this->restserver->level;
    ...
    {
        // Set header for allowed access angularjs
    ...
    protected function _detect_output_format()
    {
        ...
        if ( ! empty($this->restserver_format)) {
            return $this->restserver_format;
        }
    ...
    protected function _detect_api_key()
    {
        ...
        $this->restserver->key = NULL;
        $this->restserver->level = NULL;
        $this->restserver->user_id = NULL;
        $this->restserver->ignore_limits = FALSE;

        // Find the key from server or arguments
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            if ( ! ($row = $this->restserver->db->where(config_item('rest_key_column'), $key)->get(config_item('rest_keys_table'))->row())) {
                return FALSE;
            }

            $this->restserver->key = $row->{config_item('rest_key_column')};

            isset($row->user_id) && $this->restserver->user_id = $row->user_id;
            isset($row->level) && $this->restserver->level = $row->level;
            isset($row->ignore_limits) && $this->restserver->ignore_limits = $row->ignore_limits;
    ...
    {
        $status = $this->restserver->db->insert(config_item('rest_logs_table'), [
                    'uri' => $this->uri->uri_string(),
                    'method' => $this->request->method,
                    'params' => $this->_args ? (config_item('rest_logs_json_params') ? json_encode($this->_args) : serialize($this->_args)) : NULL,
                    'api_key' => isset($this->restserver->key) ? $this->restserver->key : '',
                    'ip_address' => $this->input->ip_address(),
                    'time' => function_exists('now') ? now() : time(),
                    'authorized' => $authorized
                ]);

        $this->_insert_id = $this->restserver->db->insert_id();
    ...
    protected function _check_limit($controller_method)
    {
        ...
        if ( ! empty($this->restserver->ignore_limits) || !isset($this->methods[$controller_method]['limit'])) {
        ...
        $result = $this->restserver->db
                ->where('uri', $this->uri->uri_string())
                ->where('api_key', $this->restserver->key)
        ...
        if ( ! $result ) {
            ...
            $this->restserver->db->insert(config_item('rest_limits_table'), [
                'uri' => $this->uri->uri_string(),
                'api_key' => isset($this->restserver->key) ? $this->restserver->key : '',
                'count' => 1,
                'hour_started' => time()
        ...
        elseif ($result->hour_started < time() - (60 * 60)) {
            ...
            $this->restserver->db
                    ->where('uri', $this->uri->uri_string())
                    ->where('api_key', isset($this->restserver->key) ? $this->restserver->key : '')
    ...
    protected function _log_access_time()
    {
        ...
        return $this->restserver->db->update(config_item('rest_logs_table'), $payload, ['id' => $this->_insert_id]);
    }
    ...
    protected function _check_access()
    {
        ...
        $this->restserver->db->select();
        $this->restserver->db->where('key', $this->restserver->key);
        $this->restserver->db->where('controller', $controller);

        $query = $this->restserver->db->get(config_item('rest_access_table'));
