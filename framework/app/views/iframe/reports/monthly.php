...
<body>
		...
		<div class="panel-heading">
			<table>
				<tbody>
					...
					<tr>
						<td>Time Start </td>
						<td>: <?php echo $month; ?> 00:00:00</td>
					</tr>
					<tr>
						<td>Time Finish </td>
						<td>: <?php echo $lastday; ?> 23:59:59</td>
					</tr>
				</tbody>
			</table>
		</div>
		<div class="alert alert-warning no-border">
			<i class="fa fa-info-circle"></i> <b>Summary</b> <br>Based on analysis and report from ID-SIRTII/CC network monitoring system. IP <?php echo $target; ?> shows some attack indication between <?php echo $month; ?> 00:00:00 and <?php echo $lastday; ?> 23:59:59 with classification as below:
		</div>
		<div class="panel-heading">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			// Signature
			$.getJSON('/reports/monthly/by_sig/graph?date=<?php echo $datequery; ?>', function(data) {
				options.series = [data];
				options.title = {
					text: 'Event by Signature <?php echo $datequery; ?>'
				};
				var chart = new Highcharts.chart("chart-signature",options);
			});
			$.getJSON('/reports/monthly/by_sig/?date=<?php echo $datequery; ?>', function(data) {
			...
			//Severity
			$.getJSON('/reports/monthly/by_severity/graph?date=<?php echo $datequery; ?>', function(data) {
				options.series = [data];
				options.title = {
					text: 'Event by Severity <?php echo $datequery; ?>'
				};
				var chart = new Highcharts.chart("chart-severity",options);
			});
			$.getJSON('/reports/monthly/by_severity/?date=<?php echo $datequery; ?>', function(data) {
				eventtotal = data.count;
				data = data.data;
				var tr = [];
				for(var i = 0;i < data.length; i++){
			...
			//IP Source
			$.getJSON('/reports/monthly/by_ip_src/graph?date=<?php echo $datequery; ?>', function(data) {
				options.series = [data];
				options.title = {
					text: 'Event by IP Source <?php echo $datequery; ?>'
				};
				var chart = new Highcharts.chart("chart-ip-src",options);
			});
			$.getJSON('/reports/monthly/by_ip_src/?date=<?php echo $datequery; ?>', function(data) {
				eventtotal = data.count;
				data = data.data;
				var tr = [];
				for(var i = 0;i < data.length; i++){
			...
			//IP Destination
			$.getJSON('/reports/monthly/by_ip_dst/graph?date=<?php echo $datequery; ?>', function(data) {
				options.series = [data];
				options.title = {
					text: 'Event by IP Destination <?php echo $datequery; ?>'
				};
				var chart = new Highcharts.chart("chart-ip-dst",options);
			});
			$.getJSON('/reports/monthly/by_ip_dst/?date=<?php echo $datequery; ?>', function(data) {
				eventtotal = data.count;
				data = data.data;
				var tr = [];
				for(var i = 0;i < data.length; i++){
